...
| 이름 | GitHub | 역할 | 담당 업무 |
|---|---|---|---|
| 김승욱 | [@seungwook-kim](https://https://github.com/seungwook-kim) | 조장 / 오케스트레이션·통합 | 의도분류, 단계 진행·재료대체 로직, Supabase 검색, HF Spaces 배포 및 통합테스트 |
...
| 하주성 | [@leeony2636](https://github.com/leeony2636)  | STT 파인튜닝 | 데이터 전처리, STT 학습데이터 구성, openai/whisper-small 파인튜닝 및 WER 평가 |
🍳 AI Human Recipe
...
| STT 도메인 파인튜닝 | 재료명, 계량단위, 진행표현, 주방 잡음 환경 학습 | Must |
| TTS 도메인 파인튜닝 | 짧은 단계 안내에 적합한 자연스러운 음성 생성 | Must |
...
짧은 조리 단계 안내에 적합한 자연스러운 발화를 목표로 파인튜닝합니다.
...
Should
STT 인식 정확도 개선 · 다양한 사용자 표현 대응 · 계량단위 인식 강화

Could
TTS 음성 톤 다양화 · 추가 STT 모델 비교 · UI 시각적 완성도 향상
...
요리 도메인 파인튜닝을 통해 기존 openai/whisper-small 대비 STT 오류율 개선
