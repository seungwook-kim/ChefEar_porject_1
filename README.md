...
(checkpoint-epoch-8, HF Hub 비공개 업로드 완료). 다만 실측 결과 HF Spaces CPU Basic 목표
응답시간(5초)을 크게 초과(평균 197초)하고 있어 배포 경로 대안을 검토 중이며, TTS→STT
재인식 검증에서도 문장별 인식 오류율(CER) 편차가 커서 원인 분석이 진행 중입니다.
