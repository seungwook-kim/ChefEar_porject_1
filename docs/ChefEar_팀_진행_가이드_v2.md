...
### 원칙 3. 조리순서는 100% 실데이터입니다 ⚠️ 완전히 바뀐 부분
~~원본 데이터(KADX 만개의레시피)에 조리순서가 없어서 Claude로 5,100건만 미리 생성했다~~ — **이 방식은 폐기되었습니다.**

동일 출처(만개의레시피)에서 조리순서가 포함된 요리명별 실데이터를 별도로 확보했습니다(60,282건, 무료·합법 확인). 요리명 다건 존재 시 조회수(INQ_CNT) 1위 기준으로 표준 레시피를 채택하며, 이 규칙으로 선정되는 모든 요리명에 대해 **LLM 생성 없이 실제 데이터로 조리순서를 제공**합니다. `source=llm_generated` 태깅이나 "제가 재구성한 순서라 실제와 다를 수 있어요" 같은 고지 문구는 이제 필요 없습니다 — 전량 `source=api_standard`입니다.
...
3. **조리순서 데이터가 왜 신뢰할 수 있는지** — 원본에 없던 걸 확인한 과정 + 별도 실데이터(60,282건)를 어떻게 확보했는지 스토리로
